...
class MyApp(ShowBase):
    def __init__(self):
        ...
        self.server = SlamServer(host='0.0.0.0', port=1234, loglevel=logging.INFO)
        Thread(target=self.server.run_forever, daemon=True).start()
    
    """
    Use asyncronously updated transformation values to update 3d camera pose.
    """
    def update_camera(self, task):
        pos = [10*num for num in self.server.pos]
